...
void redistribute_message(int sender_index, char *buf)
{
	for(int i = 0; i < MAX_CLIENTS; i++) {
		// client DNE or sender is the current client being checked
		if (NULL == CLIENTS[i] || i == sender_index) continue;
		if (fprintf(CLIENTS[i], "client %d: %s", sender_index, buf) < 0) {
			perror("fprintf");
			fclose(CLIENTS[i]);
			CLIENTS[i] = NULL;
			continue;		
...
{
	errno = 0;
	if (NULL == CLIENTS[client_index]) {
		return 0; // client already DC
	}
	...
	// successfully read a msg from client
	return 1;
}
...
void try_add_client(int server_fd)
{
	...
	FILE *client = fdopen(client_fd, "r+"); // check for this error?
	for(int i = 0; i < MAX_CLIENTS; i++) {
		if (NULL == CLIENTS[i]) {
			CLIENTS[i] = client;
			return;
	...
	fprintf(client, "Maximum number of clients are currently connected! ");
	fclose(client);
}
